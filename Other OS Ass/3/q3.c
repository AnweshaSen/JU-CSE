...
// A shared circular buffer that can hold 25 items is to be used. Each producer process stores any numbers between 1 to 80 
// (along with the producer id) in the buffer one by one and then exits. 
...
// (the consumers exit after all the data produced by all producers have been read), the parent process prints the value of TOTAL.  
